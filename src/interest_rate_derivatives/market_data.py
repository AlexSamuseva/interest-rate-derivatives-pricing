...
class MarketDataClient:
    ...
    def __init__(self, provider: str = "fred", api_key: str | None = None):
        ...
        if provider == "fred":
            ...
            self.fred_client = None  # type: ignore[assignment]
    ...
    def get_term_structure(self, date: str | None = None) -> pd.DataFrame:
        ...
        start_date = (pd.to_datetime(end_date) - datetime.timedelta(days=10)).strftime(
            "%Y-%m-%d"
        )
        ...
        for tenor, series_id in self.TENOR_MAPPING.items():
            ...
            observations = data[series_id].get("observations", [])  # type: ignore[attr-defined]
            if not observations:
        ...
        if latest_rates:
            return (
                pd.DataFrame(latest_rates, columns=["Maturity", "Rate"])
